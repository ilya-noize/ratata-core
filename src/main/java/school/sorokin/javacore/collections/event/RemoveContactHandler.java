...
public class RemoveContactHandler implements EventHandler {

    @Override
    public void processing() {
        System.out.println("Укажите параметры для удаления контакта");

        Contact template = buildContact();
        List<Contact> contacts = MANAGER.findContact(template);
        if (contacts.isEmpty()) {
            System.out.printf("Не найден контакт с такими параметрами: %s%n", template);
            return;
        }
        MANAGER.removeAll(contacts);
        System.out.println("Удалены контакты:");
